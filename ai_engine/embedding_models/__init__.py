"""
Semantic Binary Embedding pipeline for M0ST.

Pipeline:
    Function → CFG → Graph embedding → vector representation

Uses GNN-based encoders to produce function-level embedding vectors
suitable for:
...
Embeddings are stored in the Knowledge layer's EmbeddingStore.
"""
...
_NUM_OPCODE_CATS = 12  # 0-10 + 'other'=11


class BinaryEmbeddingEngine:
    """
    Produces function-level embedding vectors from CFG structure
    using GNN encoders or fallback feature averaging.
    """
    ...
    def __init__(self, graph_store, model_path: Optional[str] = None,
        ...
        if _TORCH_AVAILABLE and _gnn_available():
            try:
                ...
                print(f"[BinaryEmbeddingEngine] Could not load GNN model: {e}")
                self.model = None

    def embed_function(self, func_addr: int) -> Dict[str, Any]:
        """Produce embedding for a single function's CFG."""
    ...
    def embed_all_functions(self) -> Dict[int, Dict[str, Any]]:
        """Embed every function in the graph store."""
        results = {}
        for func in self.g.fetch_functions():
            addr = func.get("addr")
            if addr is None:
                continue
            results[addr] = self.embed_function(addr)
        return results

    def get_embedding_vector(self, func_addr: int) -> List[float]:
        """Return just the graph embedding vector."""
        result = self.embed_function(func_addr)
        return result.get("graph_embedding", [0.0] * self.embedding_dim)

    def get_embedding_b64(self, func_addr: int) -> str:
        """Return base64-encoded graph embedding."""
        result = self.embed_function(func_addr)
        return result.get("graph_embedding_b64", "")

    def find_similar_functions(self, func_addr: int, top_k: int = 5) -> List[Tuple[int, float]]:
        """Find functions most similar to the given function by embedding cosine similarity."""
        target = self.get_embedding_vector(func_addr)
        results = []
        for func in self.g.fetch_functions():
            addr = func.get("addr")
            if addr is None or addr == func_addr:
                continue
            vec = self.get_embedding_vector(addr)
    ...
    def _build_graph_data(self, func_addr: int, blocks: List[int],
        ...
        if _TORCH_AVAILABLE:
            ...
            edge_index = (torch.tensor([src_list, dst_list], dtype=torch.long)
                          if src_list else torch.zeros((2, 0), dtype=torch.long))
    ...
    def _run_gnn(self, func_addr: int, data, blocks: List[int]) -> Dict[str, Any]:
        ...
        graph_emb_b64 = base64.b64encode(
            json.dumps(graph_emb_list).encode("utf-8")).decode("ascii")
    ...
    def _fallback_embedding(self, func_addr: int, data, blocks: List[int],
        ...
        features = data["x"] if isinstance(data, dict) else (
            data.x.numpy().tolist() if _TORCH_AVAILABLE else data["x"])
        ...
        graph_emb_b64 = base64.b64encode(
            json.dumps(graph_emb).encode("utf-8")).decode("ascii")
    ...
    def _empty_result(self, func_addr: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "graph_embedding_b64": base64.b64encode(
                json.dumps(empty_vec).encode("utf-8")).decode("ascii"),
            "block_addrs": [],
            "node_count": 0,
            "edge_count": 0,
        }
